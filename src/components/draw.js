async function drawGesture(result, canvas, ui) {
  ...
  for (let gesture = 0; gesture < result.length; gesture++) {
    ...
    if (what.length > 1 && what[1].length > 0) {
      ...
      ctx.fillStyle = "black";
      if (ui.drawText) ctx.fillText(label, 8, 2 + i * ui.baseLineHeight);
      ctx.fillStyle = ui.baseLabel;
      if (ui.drawText) ctx.fillText(label, 6, 0 + i * ui.baseLineHeight);
      i += 1;
    }
  }
}
...
async function drawFace(result, canvas, ui, triangulation) {
  ...
  for (const face of result) {
    ...
    if (face.genderConfidence)
      labels.push(
        `${Math.trunc(100 * face.genderConfidence)}% ${face.gender || ""}`
      );
    ...
    if (face.emotion && face.emotion.length > 0) {
      const emotion = face.emotion.map(
        a => `${Math.trunc(100 * a.score)}% ${a.emotion}`
      );
    ...
    if (ui.drawText) {
      for (let i = 0; i < labels.length; i++) {
        ...
        ctx.fillText(
          labels[i],
          face.box[0] + face.box[2] + 9,
          (i + 1) * ui.baseLineHeight + face.box[1] + 9
        );
        ctx.fillStyle = ui.baseLabel;
        ctx.fillText(
          labels[i],
          face.box[0] + face.box[2] + 8,
          (i + 1) * ui.baseLineHeight + face.box[1] + 8
        );
      }
    }
    ctx.fillStyle = ui.baseColor;
    ...
    if (face.mesh) {
      if (ui.drawPoints) {
        for (const point of face.mesh) {
          ctx.fillStyle = ui.useDepth
            ? `rgba(${127.5 + 2 * point[2]}, ${127.5 - 2 * point[2]}, 255, 0.5)`
            : ui.baseColor;
          ctx.beginPath();
      ...
      if (ui.drawPolygons) {
        for (let i = 0; i < triangulation.length / 3; i++) {
          ...
          path.closePath();
          ctx.strokeStyle = ui.useDepth
            ? `rgba(${127.5 + 2 * points[0][2]}, ${127.5 -
                2 * points[0][2]}, 255, 0.3)`
            : ui.baseColor;
          ctx.stroke(path);
          if (ui.fillPolygons) {
            ctx.fillStyle = ui.useDepth
        ...
        if (face.annotations && face.annotations.leftEyeIris) {
          ...
          const sizeX =
            Math.abs(
              face.annotations.leftEyeIris[3][0] -
                face.annotations.leftEyeIris[1][0]
            ) / 2;
          const sizeY =
            Math.abs(
              face.annotations.leftEyeIris[4][1] -
                face.annotations.leftEyeIris[2][1]
            ) / 2;
          ctx.ellipse(
            face.annotations.leftEyeIris[0][0],
            face.annotations.leftEyeIris[0][1],
            sizeX,
            sizeY,
            0,
            0,
        ...
        if (face.annotations && face.annotations.rightEyeIris) {
          ...
          const sizeX =
            Math.abs(
              face.annotations.rightEyeIris[3][0] -
                face.annotations.rightEyeIris[1][0]
            ) / 2;
          const sizeY =
            Math.abs(
              face.annotations.rightEyeIris[4][1] -
                face.annotations.rightEyeIris[2][1]
            ) / 2;
          ctx.ellipse(
            face.annotations.rightEyeIris[0][0],
            face.annotations.rightEyeIris[0][1],
            sizeX,
            sizeY,
            0,
            0,
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (ui.drawPoints) {
      for (let pt = 0; pt < result[i].keypoints.length; pt++) {
        ctx.beginPath();
        if (ui.buffered) {
          lastDrawnPose[i].keypoints[pt].position.x =
            (lastDrawnPose[i].keypoints[pt].position.x +
              result[i].keypoints[pt].position.x) /
            2;
          lastDrawnPose[i].keypoints[pt].position.y =
            (lastDrawnPose[i].keypoints[pt].position.y +
              result[i].keypoints[pt].position.y) /
            2;
          ctx.arc(
            lastDrawnPose[i].keypoints[pt].position.x,
            lastDrawnPose[i].keypoints[pt].position.y,
  ...
  for (const hand of result) {
    ...
    if (ui.drawBoxes) {
      ...
      ctx.fillText(
        "hand",
        hand.box[0] + 3,
        1 + hand.box[1] + ui.baseLineHeight,
        hand.box[2]
      );
      ctx.fillStyle = ui.baseLabel;
      ctx.fillText(
        "hand",
        hand.box[0] + 2,
        0 + hand.box[1] + ui.baseLineHeight,
        hand.box[2]
      );
      ctx.stroke();
    }
